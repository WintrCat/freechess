...
import { EvaluatedPosition } from "./types/Position.js";
import Report from "./types/Report.js";
import { readFileSync } from "fs";
import { resolve } from "path";
import { Opening } from "./types/Position.js";
...
//FIXME: make more elegant solution to parse json in module
const openings: Opening[] = await JSON.parse(
    readFileSync(resolve("src/resources/openings.json"), "utf-8")
);
    ...
    for (let position of positions.slice(1)) {
        ...
        if (position.classification == Classification.BEST) {
            ...
            if (
                ...
                Math.abs(
                    topMove.evaluation.value - secondTopMove.evaluation.value
                ) >= 150 &&
                !isPieceHanging(
                    lastPosition.fen,
                    position.fen,
